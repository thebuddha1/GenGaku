...
<body class="antialiased">
    <div class="relative flex justify-center items-center min-h-screen bg-center bg-gray-100 dark:bg-gray-900 selection:text-white">
        <div class="text-center" style="background-color: #1f2937 !important; padding: 60px; border-radius: 30px;">
            ...
            <div id="pressContainer">
                <label id="pressCountLabel" class="text-white" style="color: white !important; font-size: 1.5rem !important; margin-bottom: 5px !important; display:none;">Quiz 0/15</label>
                <button onclick="loadNextQuiz()" id="loadQuizButton" class="font-semibold text-gray-600 bg-gray-500 rounded-lg px-6 py-3" style="color: white !important; font-size: 1rem; background-color: gray !important;">Next</button>
            </div>
            ...
            <div id="finalContainer" style="display:none;">
                ...
                <div>
                    <label id="mistakesMainLabel" class="text-white" style="color: white !important; font-size: 1.5rem; margin-bottom: 50px;">your mistakes: </label>
                </div>
                <div>
                    <form action="{{ route('save-prog-kat') }}" method="POST">
                        @csrf
        ...
        async function loadNextQuiz() {
            ...
            if (pressCount === 16) {
                ...
                try {
                    var response;
                    if (quizNumber === 1) {
                        response = await fetch('/katakana1/' + lesson);
                    } else if (quizNumber === 2) {
                        response = await fetch('/katakana2/' + lesson);
                    } else if (quizNumber === 3) {
                        response = await fetch('/katakana3/' + lesson);
                    } else if (quizNumber === 4) {
                        response = await fetch('/katakana4/' + lesson);
                    }
