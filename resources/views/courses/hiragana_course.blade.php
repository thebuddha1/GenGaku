<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white-800 leading-tight">
            {{ __('Writing course') }}
        </h2>
    </x-slot>
<!--
    -->
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="col-span-6 sm:col-span-4">
                    <x-label for="cur_hrgn" value="{{ __('Current Lesson (hiragana)') }}" />
                    <p>{{ auth()->user()->profileProgression->cur_hrgn }}</p>
                </div>
                <div class="col-span-6 sm:col-span-4">
                    <x-label for="fnshd_tsts_hir" value="{{ __('Finished tests in current lesson(hiragana)') }}" />
                    <p>{{ auth()->user()->profileProgression->fnshd_tsts_hir }}</p>
                </div>
                <h1 class="text-4xl font-semibold">Hiragana</h1>
                <p>Welcom to the Hiragana course. <br> 
                    In this page you'll be getting to know one of Japanes' 3 writing systems. <br>
                    Hiragana is one of the 2 kana systems of the Japanese language (kana meaning syllabaries, used to write Japanes phonological units, in other word morae). <br> 
                    It contains 46 symbols and an additional 63 variations of the basic symbols, all of them representing a syllable.<br>
                    Hiragana is used to write kana suffixes following a kanji root, various grammatical and function words including particles, <br>
                    and miscellaneous other native words for which there are no kanji or whose kanji form is obscure or too formal for the writing purpose. <br>
                    <br>
                    In the course there is a total of 10 lessons. In each lesson you'll be learning a set of 10-14 symbols at once. <br>
                    Before you begin the tests, you'll be able to read trough a summary in each lesson that walks you trough the symbols ahead of you, <br>
                    telling you about their pronunciation and providing some examples for it.<br>
                    When you've completed enough tests you can progress to the next one. Simple, isn't it? Let jump right into it.<br>
                </p>
                <h1 class="text-2xl font-semibold">Lesson 1</h1>
                <p>part 1: a, i, u, e, o</p>
                <p>
                    あ->a   as in father<br>
                    い->i   as in easy<br>
                    う->u   as in too<br>
                    え->e   as in pay<br>
                    お->o   as in open<br>
                </p>
                <p>part 2: ka, ki, ku, ke, ko</p>
                <p>
                    か->ka   as in cut<br>
                    き->ki   as in keep<br>
                    く->ku   as in cool<br>
                    け->ke   as in cat<br>
                    こ->ko   as in cold<br>
                </p>
                <a href="/quiz-hir?lesson=1"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 2</h1>
                <p>part 1: sa, shi, su, se, so</p>
                <p>
                    さ->sa   as in sun<br>
                    し->shi   as in she<br>
                    す->su   as in suit<br>
                    せ->se   as in set<br>
                    そ->so   as in soap<br>
                </p>
                <p>part 2: ta, chi, tsu, te, to</p>
                <p>
                    た->ta   as in tall<br>
                    ち->chi   as in cheese<br>
                    つ->tsu   as in tsunami<br>
                    て->te   as in ten<br>
                    と->to   as in toe<br>
                </p>
                <a href="/quiz-hir?lesson=2"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 3</h1>
                <p>part 1: na, ni, nu, ne, no, n</p>
                <p>
                    な->na   as in nut<br>
                    に->ni   as in knee<br>
                    ぬ->nu   as in new<br>
                    ね->ne   as in net<br>
                    の->no   as in note<br>
                    ん->n   as in sung<br>
                </p>
                <p>part 2: ha, hi, fu, he, ho, ya, yu, yo</p>
                <p>
                    は->ha   as in hot<br>
                    ひ->hi   as in heat<br>
                    ふ->fu   as in food<br>
                    へ->he   as in head<br>
                    ほ->ho   as in hope<br>
                    や->ya   as in yard<br>
                    ゆ->yu   as in you<br>
                    よ->yo   as in yo-yo<br>
                </p>
                <a href="/quiz-hir?lesson=3"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 4</h1>
                <p>part 1: ma, mi, mu, me, mo</p>
                <p>
                    ま->ma   as in mop<br>
                    み->mi   as in meet<br>
                    む->mu   as in mood<br>
                    め->me   as in met<br>
                    も->mo   as in more<br>
                </p>
                <p>part 2: ra, ri, ru, re, ro, wa, wo</p>
                <p>
                    ら->ra   as in run<br>
                    り->ri   as in read<br>
                    る->ru   as in roof<br>
                    れ->re   as in red<br>
                    ろ->ro   as in row<br>
                    わ->wa   as in water<br>
                    を->wo   as in won<br>
                </p>
                <a href="/quiz-hir?lesson=4"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 5</h1>
                <p>part 1: ga, gi, gu, ge, go</p>
                <p>
                    が->ga   as in garden<br>
                    ぎ->gi   as in gift<br>
                    ぐ->gu   as in good<br>
                    げ->ge   as in get<br>
                    ご->go   as in go<br>
                </p>
                <p>part 2: za, ji, zu, ze, zo</p>
                <p>
                    ざ->za   as in zoo<br>
                    じ->ji   as in jeep<br>
                    ず->zu   as in zebra<br>
                    ぜ->ze   as in zero<br>
                    ぞ->zo   as in zone<br>
                </p>
                <a href="/quiz-hir?lesson=5"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 6</h1>
                <p>part 1: da, ji, zu, de, do</p>
                <p>
                    だ->da   as in dad<br>
                    ぢ->ji   as in g in gin<br>
                    づ->zu   as in heads<br>
                    で->de   as in day<br>
                    ど->do   as in dog<br>
                </p>
                <p>part 2: ba, bi, bu, be, bo</p>
                <p>
                    ば->ba   as in bat<br>
                    び->bi   as in big<br>
                    ぶ->bu   as in blue<br>
                    べ->be   as in bed<br>
                    ぼ->bo   as in boat<br>
                </p>
                <a href="/quiz-hir?lesson=6"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 7</h1>
                <p>part 1: pa, pi, pu, pe, po</p>
                <p>
                    ぱ->pa   as in part<br>
                    ぴ->pi   as in pink<br>
                    ぷ->pu   as in pool<br>
                    ぺ->pe   as in pen<br>
                    ぽ->po   as in pole<br>
                </p>
                <p>part 2: kya, kyu, kyo</p>
                <p>
                    きゃ->kya   as in c in cat<br>
                    きゅ->kyu   as in queue<br>
                    きょ->kyo   as in cue<br>
                </p>
                <p>part 3: sha, shu, sho</p>
                <p>
                    しゃ->sha   as in shop<br>
                    しゅ->shu   as in shoot<br>
                    しょ->sho   as in shore<br>
                </p>
                <p>part 4: cha, chu, cho</p>
                <p>
                    ちゃ->cha   as in chart<br>
                    ちゅ->chu   as in chew<br>
                    ちょ->cho   as in choke<br>
                </p>
                <a href="/quiz-hir?lesson=7"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 8</h1>
                <p>part 1: nya, nyu, nyo</p>
                <p>    
                    にゃ->nya   as in knee<br>
                    にゅ->nyu   as in new<br>
                    にょ->nyo   as in know<br>
                <p>part 2: nhya, hyu, hyo</p>
                <p>    
                    ひゃ->hya   as in hue<br>
                    ひゅ->hyu   as in hugh<br>
                    ひょ->hyo   as in hue<br>
                <p>part 3: mya, myu, myo</p>
                <p>    
                    みゃ->mya   as in mia<br>
                    みゅ->myu   as in miu<br>
                    みょ->myo   as in meow<br>
                <p>part 4: rya, ryu, ryo</p>
                <p>
                    りゃ->rya   as in ree<br>
                    りゅ->ryu   as in ryou<br>
                    りょ->ryo   as in rio<br>
                </p>
                <a href="/quiz-hir?lesson=8"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 9</h1>
                <p>part 1: gya, gyu, gyo</p>
                <p>
                    ぎゃ->gya   as in ga<br>
                    ぎゅ->gyu   as in gyou<br>
                    ぎょ->gyo   as in go<br>
                </p>
                <p>part 2: ja, ju, jo</p>
                <p>
                    じゃ->ja   as in jay<br>
                    じゅ->ju   as in july<br>
                    じょ->jo   as in joe<br>
                </p>
                <p>part 3: bya, byu, byo</p>
                <p>
                    びゃ->bya   as in by a<br>
                    びゅ->byu   as in by u<br>
                    びょ->byo   as in by o<br>
                </p>
                <p>part 4: pya, pyu, pyo</p>
                <p>
                    ぴゃ->pya   as in py a<br>
                    ぴゅ->pyu   as in py u<br>
                    ぴょ->pyo   as in py o<br>
                </p>
                <a href="/quiz-hir?lesson=9"><button type="button">Start Test</button></a>
                <h1 class="text-2xl font-semibold">Lesson 10</h1>
                <p>part 13: double consonants</p>
                <p>
                    っか->kka   as in cat<br>
                    っす->ssu   as in kiss<br>
                    っと->tto   as in bet<br>
                    っぷ->ppu   as in lip<br>
                    っり->rri   as in tree<br>
                </p>
                <p>extended vowels</p>
                <p>    
                    ああ->aa   as in baa<br>
                    いい->ii   as in beef<br>
                    うう->uu   as in boot<br>
                    ええ->ee   as in beet<br>
                    おお->oo   as in boat<br>
                </p>
                <a href="/quiz-hir?lesson=10"><button type="button">Start Test</button></a>
            </div>
        </div>
    </div>
</x-app-layout>